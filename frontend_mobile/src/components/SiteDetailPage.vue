<template>
  <div class="">
    ...
    <div class="page-content pb-0">
        ...
        <div class="card card-style" style="margin-top:-40px; z-index:1">
            <div class="content text-center mt-4">
                ...
                <div v-if="getDiveSiteByUniqueName.waterTemperature" class="evaluation d-flex mt-2">
                    <img :src="'https://d34l91104zg4p3.cloudfront.net/assets/weather/'+(weatherIcon[getDiveSiteByUniqueName.waterTemperature.weatherDescription])+'.png'" width="50"/>
                    <div>
        ...
        <div v-if="getDiveSitesNearby && getDiveSitesNearby.length > 0">
            <h4 class="text-start mb-2 mt-4" style="margin-left: 10px;margin-right: 10px;">근처 사이트</h4>
            <div class="splide slider-no-arrows visible-slider slider-no-dots" id="single-slider-nearby">
                <div class="splide__track">
                    <div class="splide__list">
                        <div v-for="near in getDiveSitesNearby" class="splide__slide">
                    ...
                    <table class="table table-borderless text-center rounded-sm shadow-l mt-3" style="overflow: hidden;">
                        ...
                        <tbody v-if="getDiveSiteByUniqueName.waterTemperature && getDiveSiteByUniqueName.waterTemperature.temperatureDetail && getDiveSiteByUniqueName.waterTemperature.temperatureDetail.MinC">
                            <tr class="border-bottom" v-for="(minC, index) in getDiveSiteByUniqueName.waterTemperature.temperatureDetail.MinC">
                                ...
                                <td class="font-12"><img class="me-2" :src="'https://d34l91104zg4p3.cloudfront.net/assets/'+(getDiveSiteByUniqueName['month'+(index+1)].filter(x=>x.type=='climate').length > 0 ? enumClimate[getDiveSiteByUniqueName['month'+(index+1)].filter(x=>x.type=='climate')[0].title] : 'empty')+'.svg'" width="20" height="20"/>{{ ((getDiveSiteByUniqueName.waterTemperature.temperatureDetail.MaxC[index]+minC)/2).toFixed(1) }}ºC</td>
                                <td class=""><img class="img_pop" :src="'https://d34l91104zg4p3.cloudfront.net/assets/'+(getDiveSiteByUniqueName['month'+(index+1)].filter(x=>x.type=='popularity').length > 0 ? enumPopularity[getDiveSiteByUniqueName['month'+(index+1)].filter(x=>x.type=='popularity')[0].title] : 'empty')+'.svg'" width="32" height="32"/></td>
                            </tr>
                        </tbody>
                    </table>
                    <div class="text-end">
        ...
        <div class="card card-style">
            ...
            <a :href="'/review2/'+getDiveSiteByUniqueName._id" class="color-highlight font-12 wedive-txt-all">모두보기</a>
            <div class="divider mt-3 mb-2"></div>
                <div v-if="getDiveSiteByUniqueName.reviews && getDiveSiteByUniqueName.reviews.length>0" class="splide single-slider slider-no-arrows slider-has-dots pb-2 mb-0 me-n2 ms-n2" id="single-slider-review">
                    <div class="splide__track">
                        <div class="splide__list">
                            <div v-for="review in getDiveSiteByUniqueName.reviews.filter(x=>x.isBlocked==false)" class="splide__slide pt-2">
                            ...
                            <div class="splide__slide">
                                <a :href="'/review2/'+getDiveSiteByUniqueName._id">
                                <div class="min-h-230 p-2">
...
export default {
  ...
  apollo: {
      ...
      getDiveSitesNearby: {
          ...
          variables() {
              return {
                lat1: (this.getDiveSiteByUniqueName.latitude-0.2),
                lon1: (this.getDiveSiteByUniqueName.longitude-0.2),
                lat2: (this.getDiveSiteByUniqueName.latitude+0.2),
                lon2: (this.getDiveSiteByUniqueName.longitude+0.2),
              }
          },
          result() {
              var cnt = 0;
              this.getDiveSitesNearby.forEach(x => {
                  if (x._id == this.getDiveSiteByUniqueName._id) {
                      this.getDiveSitesNearby.splice(cnt, 1);
                  }
                  cnt++;
              });
      ...
      getDiveSiteByUniqueName: {
          ...
          `,
          skip() {
              return (this.getDiveSiteByUniqueName && this.getDiveSiteByUniqueName._id ? true : false)
          },
          variables() {
              return {
  ...
  methods: {
      ...
      findBuddyClick() {
          ...
          location.href = '/buddy/create?site=' + this.getDiveSiteByUniqueName.uniqueName;
      },
...
.ico_feature19 {width: 44px;height: 40px;background-position: 0px -114px;}
.ico_feature20 {width: 44px;height: 40px;background-position: -45px -114px;}
.ico_feature21 {width: 44px;height: 40px;background-position: -90px -114px;}
.ico_feature22 {width: 44px;height: 40px;background-position: -135px -114px;}
.ico_feature23 {width: 44px;height: 40px;background-position: -180px -114px;}
.ico_feature24 {width: 44px;height: 40px;background-position: -225px -114px;}
.ico_feature25 {width: 44px;height: 40px;background-position: 0px -152px;}
.ico_feature26 {width: 44px;height: 40px;background-position: -45px -152px;}
.ico_feature27 {width: 44px;height: 40px;background-position: -90px -152px;}
.ico_feature28 {width: 44px;height: 40px;background-position: -135px -152px;}
